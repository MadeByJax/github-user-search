  ...
  return (
    <div className="shadow-app bg-white dark:bg-app-grey-3 mt-4 rounded-xl py-8 px-6 md:p-12 dark:text-white">
      <div className="flex gap-8">
        ...
        <div className="md:grid grid-cols-2 w-full">
          <div className="col-span-2 md:col-span-1">
            {user.name ? (
              <h3 className="font-bold text-base md:text-2xl">{user.name}</h3>
            ) : (
              <h3 className="font-bold text-base md:text-2xl">{user.login}</h3>
            )}
            <h4 className="text-xs md:text-base  text-app-primary">{`@${user.login}`}</h4>
          </div>
          <div className="md:col-span-1">
            <p className="text-xs text-app-grey-1 dark:text-white md:text-base md:text-right">
              {user.created_at}
            </p>
          </div>
        </div>
      ...
      <div className="md:ml-[150px]">
        {user.bio ? (
          <p className="text-sm text-app-grey-2 dark:text-white leading-6 mt-8 md:mt-[-40px]">
            {user.bio}
          </p>
        ) : (
          <p className="text-sm text-app-grey-2 dark:text-white leading-6 w-full mt-8 md:mt-[-40px]">
            This profile has no bio
          </p>
        )}
      </div>
      <div className="flex md:ml-[150px]  justify-between p-5 bg-app-light dark:bg-app-dark rounded-xl mt-6">
      ...
      <ul className="md:ml-[150px] grid md:grid-cols-2 gap-4 mt-6">
        ...
        <li className="flex gap-5">
          ...
          <a className="text-xs text-app-grey-2 dark:text-white" href="">
            @aurohs
          </a>
        </li>
        <li className="flex gap-5">
