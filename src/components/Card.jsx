...
const Card = ({ user, isLoading }) => {
  return (
    <div className="bg-card-blue mt-4 rounded-xl py-8 px-6 md:p-12 text-white">
      <div className="flex gap-5">
        ...
        <div className="md:grid grid-cols-2 w-full">
          <div className="col-span-2 md:col-span-1">
            {user.name ? (
              <h3 className="text-base md:text-2xl">{user.name}</h3>
            ) : (
              <h3 className="text-base md:text-2xl">{user.login}</h3>
            )}
            <h4 className="text-xs md:text-base  text-accent-blue">{`@${user.login}`}</h4>
          </div>
          <div className="md:col-span-1">
            <p className="text-xs md:text-base md:text-right">
              Joined 25 Jan 2011
            </p>
          </div>
        </div>
      ...
      <div className="md:ml-[137px]">
        {user.bio ? (
          <p className="text-sm leading-6 mt-8 md:mt-[-40px]">{user.bio}</p>
        ) : (
          <p className="text-sm leading-6 w-full mt-8 md:mt-[-40px]">
            This profile has no bio
          </p>
        )}
      </div>
      <div className="flex md:ml-[137px]  justify-between p-5 bg-background-blue rounded-xl mt-6">
        <div className="flex gap-2 flex-col items-center">
          <p className="text-xs">Repos</p>
          <h2 className="">{user.public_repos}</h2>
        </div>
        <div className="flex gap-2 flex-col items-center">
      ...
      <ul className="md:ml-[137px] grid md:grid-cols-2 gap-4 mt-6">
